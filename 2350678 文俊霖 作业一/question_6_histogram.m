...
for idx = 1:length(ranges)
    ...
    tb = readtable("C:\Users\28022\Desktop\作业一数据(不宜改动).xlsx", opts, "UseExcel", false);
    tbl = [tbl; tb]; %#ok<AGROW>
end
...
% 设置横纵坐标标签
xlabel('车辆的平均有效车长 (m)', 'Interpreter', 'latex', 'FontSize', 14);  % 横坐标
ylabel('概率密度', 'Interpreter', 'latex', 'FontSize', 14);  % 纵坐标
grid on;  % 打开网格线
