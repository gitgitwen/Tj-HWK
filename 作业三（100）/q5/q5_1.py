...
# 读取平滑后的数据
input_file = 'smoothed_data.xlsx'  # 输入文件名
...
# 绘制移动平均数据
plt.plot(df['FDT_TIME'], df['MA_3'], label='window_3', color='#FF6347', linewidth=1)
plt.plot(df['FDT_TIME'], df['MA_5'], label='window_5', color='#66CDAA', linewidth=1)
plt.plot(df['FDT_TIME'], df['MA_7'], label='window_7', color='#FFD700', linewidth=1)
...
# 保存图表为 PNG 图片
output_image = 'speed_time_series_hm.png'
